...
/*
	Imitate
	curl -X POST \
  	https://staging.ntp.net.in/api/api-manager/v1/consumer/mobile_app_openrap/credential/register \
  	-H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI3YWIyMmJkMTNlZTQ0N2Q3YWM4ZmYwYjAxNzhmNjA1MyJ9.RM50L4adtatGwth1aMmxFnsQReOFMbrS1UuQPR1Xdi4' \
...
let registerDikshaApp = () => {
	...
	let initialToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiI3YWIyMmJkMTNlZTQ0N2Q3YWM4ZmYwYjAxNzhmNjA1MyJ9.RM50L4adtatGwth1aMmxFnsQReOFMbrS1UuQPR1Xdi4'
	let callableUrl = 'https://staging.ntp.net.in/api/api-manager/v1/consumer/mobile_app_openrap/credential/register'
	let headers = {
...
/*
	Must mimic
	curl -X POST \
  	https://staging.ntp.net.in/api/api-manager/v1/consumer/mobile_device_openrap/credential/register \
...
let registerdikshaDevice = (token) => {
	...
	const pattern = 'YYYY-MM-DD HH:mm:ss:SSSZZ';
	const timestamp = getTimestamp(pattern);
	let callableUrl = 'https://staging.ntp.net.in/api/api-manager/v1/consumer/mobile_device_openrap/credential/register'
	let headers = {
...
let generateOriginalJWTs = () => {
	...
	.then(value => {
		key = value.key;
		secret = value.secret;
		console.log("Diksha app registered");
		...
		key = value.key;
		secret = value.secret;
		console.log("Diksha device registered");
		return generateJwt(key, secret);
	}).then(value => {
        token = value.token;
